...
def save_transit_data(stop_times_bsag_updates):
    ...
    print(prefix + f"Verspätungsdaten ({transit_max}) erfolgreich in {toc - tic:0.2f} Sekunden hochgeladen...")
...
def save_traffic_data(traffic_data_bsag_updates):
    ...
    dbc.disconnect(conn)
    toc = time.perf_counter()
    print(prefix + f"Daten ({traffic_max}) erfolgreich in {toc - tic:0.2f} Sekunden hochgeladen...")
...
def save_events_data(events_bsag_updates_df):
    ...
    dbc.disconnect(conn)
    toc = time.perf_counter()
    print(prefix + f"Daten ({events_max}) erfolgreich in {toc - tic:0.2f} Sekunden hochgeladen...")
...
def save_weather_data(weather_bremen_df):
    ...
    dbc.disconnect(conn)
    toc = time.perf_counter()
    print(prefix + f"Daten ({weather_max}) erfolgreich in {toc - tic:0.2f} Sekunden hochgeladen...")
...
async def run_weather_data_async():
    save_weather_data(
        get_weather_dataframe("https://api.openweathermap.org/data/2.5/weather", "131b00cd42bee49451a4c69d496797e1",
                              "Bremen"))
